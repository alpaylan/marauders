...
pub fn convert_file(path: &Path, target: ConversionTarget) -> Result<PathBuf, ApiError> {
    ...
    match target {
        ...
        ConversionTarget::Patch => {
            ...
            let (base_source, patches) =
                crate::syntax::patch::render_etna_patches_from_comment(&spans, &source_rel_path)
                    .map_err(|e| ApiError::ProjectError(e.to_string()))?;

            std::fs::write(path, &base_source)?;

            // Etna-style: a flat `patches/` directory at the project root.
            ...
            for file in &patches {
                let dst = patches_dir.join(format!("{}.patch", file.variant_id));
                std::fs::write(&dst, &file.content)?;
                last_written = Some(dst);
            }
            return Ok(last_written.unwrap_or(patches_dir));
        }
        ...
        ConversionTarget::Comment => {
                    ...
                    .map_err(|e| ApiError::ProjectError(e.to_string()))?
            {
                ...
                // root. The patches were generated by `convert -t patch` and
                // we splice their hunks back into the source as marauders
                // comment blocks.
                std::fs::write(path, converted)?;
                return Ok(path.to_path_buf());
            } else {
